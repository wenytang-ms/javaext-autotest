...
| Navigation / code intelligence | `waitForLanguageServer`, `goToLine <n>`, `goToEndOfLine`, `findText <text>`, `navigateToError <n>`, `applyCodeAction <label>`, `renameSymbol <newName>`, `organizeImports`, `triggerCompletion`, `triggerCompletionAt <place>`, `hoverOnText <text>`, `dismissHover` | Java LS, Problems, Code Actions, completion, rename, and hover |
...
| TreeView | `click <name> tree item`, `expandTreeItem <name>`, `doubleClick <name> tree item`, `clickTreeItemAction <item> <label>`, `contextMenu <item> <menuLabel>`, `createNewFile <folder> <name>`, `openDependencyExplorer` | Tree node click/expand/double-click, inline actions, context menus, and Java Dependencies |
